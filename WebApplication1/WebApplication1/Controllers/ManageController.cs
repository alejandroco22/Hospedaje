...
namespace WebApplication1.Controllers
{
    ...
    public class ManageController : Controller
    {
        ...
        public async Task<ActionResult> AddPhoneNumber(AddPhoneNumberViewModel model)
        {
            ...
            var code = await UserManager.GenerateChangePhoneNumberTokenAsync(User.Identity.GetUserId(), model.telefono);
            if (UserManager.SmsService != null)
            {
                var message = new IdentityMessage
                {
                    Destination = model.telefono,
                    Body = "Su código de seguridad es: " + code
                };
                await UserManager.SmsService.SendAsync(message);
            }
            return RedirectToAction("VerifyPhoneNumber", new { PhoneNumber = model.telefono });
        }
        ...
        public async Task<ActionResult> VerifyPhoneNumber(string phoneNumber)
        {
            ...
            return phoneNumber == null ? View("Error") : View(new VerifyPhoneNumberViewModel { telefono = phoneNumber });
        }
        ...
        public async Task<ActionResult> VerifyPhoneNumber(VerifyPhoneNumberViewModel model)
        {
            ...
            var result = await UserManager.ChangePhoneNumberAsync(User.Identity.GetUserId(), model.telefono, model.Code);
            if (result.Succeeded)
            {
                var user = await UserManager.FindByIdAsync(User.Identity.GetUserId());
                if (user != null)
